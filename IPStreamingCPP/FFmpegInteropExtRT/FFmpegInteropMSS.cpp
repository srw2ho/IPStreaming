...
//	avformat_network_init();
	...
	//	av_register_all();
		avformat_network_init();
	// wsc no: multitasking	av_lockmgr_register(lock_manager);
		isRegistered = true;
	}
	m_isFirstSeek = true;
}
...
{
	mutexGuard.lock();
	m_ptimeouthandler->setTimeoutBreak(true); // Break Timeout alway set to timeout = 1
	if (mss)
	{
		mss->Starting -= startingRequestedToken;
		mss->SampleRequested -= sampleRequestedToken;
		mss->Closed -= MediaClosedToken;
		mss = nullptr;
	}

	if (m_pOutPutEncoding != nullptr) {
		m_pOutPutEncoding->stopEncoding();
		delete m_pOutPutEncoding;
	}
	// Clear our data
...
FFmpegInteropMSS^ FFmpegInteropMSS::StartFFmpegInteropMSSFromStream(IRandomAccessStream^ stream, bool forceAudioDecode, bool forceVideoDecode, PropertySet^ ffmpegOptions) {
	...
	if (FAILED(this->CreateMediaStreamSource(stream, forceAudioDecode, forceVideoDecode, ffmpegOptions, mss)))
	{
...
FFmpegInteropMSS^ FFmpegInteropMSS::StartFFmpegInteropMSSFromUri(String^ uri, bool forceAudioDecode, bool forceVideoDecode, PropertySet^ ffmpegOptions) {

	if (FAILED(this->CreateMediaStreamSource(uri, forceAudioDecode, forceVideoDecode, ffmpegOptions)))
	{
		// We failed to initialize, clear the variable to return failure
		delete this;
		return nullptr;
...
HRESULT FFmpegInteropMSS::CreateMediaStreamSource(String^ uri, bool forceAudioDecode, bool forceVideoDecode, PropertySet^ ffmpegOptions)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		avFormatCtx->interrupt_callback.callback = &timeout_handler::check_interrupt;

		m_ptimeouthandler->setTimeoutinMS(1*7000); // 2*7000ms timeout
...
HRESULT FFmpegInteropMSS::InitFFmpegContext(bool forceAudioDecode, bool forceVideoDecode)
{
	...
	if (SUCCEEDED(hr))
	{
		m_pOutPutEncoding = new MediaSampleOutputEncoding(this->avFormatCtx);
		m_pReader = ref new FFmpegReader(avFormatCtx,this->m_ptimeouthandler);
		if (m_pReader == nullptr)
		{
			hr = E_OUTOFMEMORY;
		}
...
MediaThumbnailData ^ FFmpegInteropExtRT::FFmpegInteropMSS::ExtractThumbnail()
{
...
HRESULT FFmpegInteropMSS::CreateAudioStreamDescriptor(bool forceAudioDecode)
{
	...
	else
	{
		...
		audioSampleProvider = ref new UncompressedAudioSampleProvider(m_pReader, avFormatCtx, avAudioCodecCtx, this->m_pOutPutEncoding);
	}
...
HRESULT FFmpegInteropMSS::CreateVideoStreamDescriptor(bool forceVideoDecode)
{
	...
	if (avVideoCodecCtx->codec_id == AV_CODEC_ID_H264 && !forceVideoDecode)
	{
		...
		{
			videoSampleProvider = ref new H264AVCSampleProvider(m_pReader, avFormatCtx, avVideoCodecCtx, this->m_pOutPutEncoding);
		}
		else
		{
	...
	else if (avVideoCodecCtx->codec_id == AV_CODEC_ID_HEVC && Windows::Foundation::Metadata::ApiInformation::IsMethodPresent("Windows.Media.MediaProperties.VideoEncodingProperties", "CreateHevc") && !forceVideoDecode)
	{
		...
		if (avVideoCodecCtx->extradata != nullptr && avVideoCodecCtx->extradata_size > 22 &&
			...
			videoSampleProvider = ref new HEVCSampleProvider(m_pReader, avFormatCtx, avVideoCodecCtx, this->m_pOutPutEncoding);
		}
		else
		{
			videoSampleProvider = ref new H264SampleProvider(m_pReader, avFormatCtx, avVideoCodecCtx, this->m_pOutPutEncoding);
	...
	else
	{
		...
		videoSampleProvider = ref new UncompressedVideoSampleProvider(m_pReader, avFormatCtx, avVideoCodecCtx, this->m_pOutPutEncoding);
...
void FFmpegInteropMSS::OnStarting(MediaStreamSource ^sender, MediaStreamSourceStartingEventArgs ^args)
{
	...
	if (request->StartPosition && request->StartPosition->Value.Duration <= mediaDuration.Duration && (!m_isFirstSeek || request->StartPosition->Value.Duration > 0))
	{
...
/*
		if (videoSampleProvider && !videoSampleProvider->IsEnabled)
		{
			videoSampleProvider->EnableStream();
		}

		if (audioSampleProvider && !audioSampleProvider->IsEnabled)
		{
...
void FFmpegInteropMSS::OnStarting(MediaStreamSource ^sender, MediaStreamSourceStartingEventArgs ^args)
{
	...
	if (request->StartPosition && request->StartPosition->Value.Duration <= mediaDuration.Duration)
	{
		...
		if (streamIndex >= 0)
		{
			if (request->StartPosition->Value.Duration == 0)
			{
				timeposition.Duration = { LONGLONG(avFormatCtx->start_time * 10000000 / double(AV_TIME_BASE)) };;
			}
			else timeposition = request->StartPosition->Value;
	...
	//		int64_t seekTarget = static_cast<int64_t>(request->StartPosition->Value.Duration / (av_q2d(avFormatCtx->streams[streamIndex]->time_base) * 10000000));
			int64_t seekTarget = static_cast<int64_t>(timeposition.Duration / (av_q2d(avFormatCtx->streams[streamIndex]->time_base) * 10000000));
		...
		//request->SetActualStartPosition(request->StartPosition->Value);
		request->SetActualStartPosition(timeposition);
	}
}
*/
