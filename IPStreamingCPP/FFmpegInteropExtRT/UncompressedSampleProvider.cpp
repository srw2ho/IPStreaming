...
UncompressedSampleProvider::UncompressedSampleProvider(FFmpegReader^ reader, AVFormatContext* avFormatCtx, AVCodecContext* avCodecCtx, MediaSampleOutputEncoding* pOutputEncoding)
...
//	av_packet_rescale_ts(pPacket, m_pAvFormatCtx->streams[pPacket->stream_index]->time_base, this->m_pAvCodecCtx->time_base);
	...
	while (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			fGotFrame = true;
			this->WriteAVFrameToMuxer(m_pAvFrame, avPacket, streamIdx);
