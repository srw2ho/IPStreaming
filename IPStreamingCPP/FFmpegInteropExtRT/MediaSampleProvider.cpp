...
MediaSampleProvider::MediaSampleProvider(
	...
	MediaSampleOutputEncoding* pOutputEncoding)
	: m_pReader(reader)
...
HRESULT MediaSampleProvider::GetNextPacket(DataWriter ^ writer, LONGLONG & pts, LONGLONG & dur, bool allowSkip)
{
	HRESULT hr = S_OK;
...
void MediaSampleProvider::WriteAVPacketToMuxer(AVPacket* avPacket) {
	if (m_pOutputEncoding != nullptr) {

		m_pOutputEncoding->WriteAVPacketToMuxer(avPacket);
	}
}

void MediaSampleProvider::WriteAVFrameToMuxer(AVFrame* avFame, AVPacket* avPacket, int streamIdx) {
	if (m_pOutputEncoding != nullptr) {

		m_pOutputEncoding->WriteAVFrameToMuxer(avFame, avPacket, streamIdx);
	}
}
